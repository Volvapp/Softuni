...
import org.productshopex.data.repositories.ProductRepository;
import org.productshopex.data.repositories.UserRepository;
import org.productshopex.service.ProductService;
import org.productshopex.service.dtos.ProductInRangeDto;
import org.productshopex.service.dtos.ProductSeedDto;
...
public class ProductServiceImpl implements ProductService {
    ...
    public void seedProducts() throws FileNotFoundException {
        ...
        for (ProductSeedDto productSeedDto : productSeedDtos) {
            if (this.validationUtil.isValid(productSeedDto)) {
                this.validationUtil.getViolations(productSeedDtos)
    ...
    public List<ProductInRangeDto> getAllProductsInRange(BigDecimal from, BigDecimal to) {
        return this.productRepository.findAllByPriceBetweenAndBuyerIsNullOrderByPrice(from, to)
                .stream().map(p -> {
                    ProductInRangeDto map = this.modelMapper.map(p, ProductInRangeDto.class);
                    map.setSeller(p.getSeller().getFirstName() + " " + p.getSeller().getLastName());
                    return map;
                })
    ...
    private Set<Category> getRandomCategories() {
        ...
        for (int i = 0; i < randomCount; i++) {
            long randId = ThreadLocalRandom.current().nextLong(1, this.categoryRepository.count() + 1);
            categories.add(this.categoryRepository.findById(randId).get());
        }
        return categories;
    }
